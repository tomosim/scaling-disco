...
import {
	...
	ProfileSchema,
	SearchSchema,
	PaginationStructureSchema,
} from "../utils/schemas";
import z from "zod";

const SearchResponseSchema = PaginationStructureSchema.extend({
	items: SearchSchema.array(),
});
	...
	useEffect(() => {
		const fetchProfiles = async () => {
			try {
				...
				const searchData = SearchResponseSchema.parse(
				...
				const profilesData = ProfileResponseSchema.parse(
